...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setProjectData(data);
    setEditableStr(data?.groups?.[groupRowIndexNumber]?.groupName || `工廠 #${groupRowIndexNumber + 1}`);
  ...
  return (
    <div>
      ...
      <HiddenInput name={["groups", groupRowIndexNumber, "groupName"]} initialValue={`工廠 #${groupRowIndexNumber + 1}`} required />
      <Table
        className="mt-4"
        expandable={{
      ...
      {!!editable && (
        <Row justify="space-between">
          <Button className="mt-4" type="primary" danger onClick={handleDelete(groupRowIndex)}>
