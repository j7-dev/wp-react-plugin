<?php // phpcs:disable
// remember to change the namespace to your plugin's namespace
...
if ( ! class_exists( __NAMESPACE__ . '\TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			 ...
			 */
			\add_action( 'init', array( $this, 'load_textdomain' ), 15 );
...
<div class="tgmpa wrap">
				...
				if ( ! \current_user_can( 'activate_plugins' ) && ! \current_user_can( 'install_plugins' ) && ! \current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$rendered .= sprintf(
							$line_template,
							sprintf(
								\translate_nooped_plural( $this->strings[ $type ], $count, 'tgmpa' ),
								$imploded,
								$count
							)
						);
		...
		protected function create_user_action_links_for_notice( $install_count, $update_count, $activate_count, $line_template ) {
			...
			if ( \current_user_can( 'install_plugins' ) ) {
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						$link_template,
						\translate_nooped_plural( $this->strings['install_link'], $install_count, 'tgmpa' ),
				...
				if ( $update_count > 0 ) {
					$action_links['update'] = sprintf(
						$link_template,
						\translate_nooped_plural( $this->strings['update_link'], $update_count, 'tgmpa' ),
						\esc_url( $this->get_tgmpa_status_url( 'update' ) )
					);
				}
			}
			...
			if ( \current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					$link_template,
					\translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'tgmpa' ),
	...
	if ( ! function_exists( __NAMESPACE__ . '\load_tgm_plugin_activation' ) ) {
		/**
		...
		function load_tgm_plugin_activation() {
			$GLOBALS[ __NAMESPACE__ . '\tgmpa' ] = TGM_Plugin_Activation::get_instance();
		}
	}
...
if ( ! function_exists( __NAMESPACE__ . '\tgmpa' ) ) {
	/**
	...
	function tgmpa( $plugins, $config = array() ) {
		$instance = call_user_func( array( get_class( $GLOBALS[ __NAMESPACE__ . '\tgmpa' ] ), 'get_instance' ) );
...
if ( ! class_exists( __NAMESPACE__ . '\TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends \WP_List_Table {
		 ...
		 */
		public function __construct() {
			$this->tgmpa = call_user_func( array( get_class( $GLOBALS[ __NAMESPACE__ . '\tgmpa' ] ), 'get_instance' ) );
...
if ( ! class_exists( __NAMESPACE__ . '\TGM_Bulk_Installer' ) ) {
...
if ( ! class_exists( __NAMESPACE__ . 'TGM_Bulk_Installer_Skin' ) ) {
...
if ( ! function_exists( __NAMESPACE__ . '\tgmpa_load_bulk_installer' ) ) {
	...
	function tgmpa_load_bulk_installer() {
		...
		if ( ! isset( $GLOBALS[ __NAMESPACE__ . '\tgmpa' ] ) ) {
			return;
		}

		// Get TGMPA class instance.
		$tgmpa_instance = call_user_func( array( get_class( $GLOBALS[ __NAMESPACE__ . '\tgmpa' ] ), 'get_instance' ) );
		...
		if ( isset( $_GET['page'] ) && $tgmpa_instance->menu === $_GET['page'] ) {
			...
			if ( ! class_exists( __NAMESPACE__ . '\TGMPA_Bulk_Installer' ) ) {
			...
			if ( ! class_exists( __NAMESPACE__ . '\TGMPA_Bulk_Installer_Skin' ) ) {
...
if ( ! class_exists( __NAMESPACE__ . '\TGMPA_Utils' ) ) {
